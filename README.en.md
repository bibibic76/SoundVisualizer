...
- **Not every sound is captured.** Only media and game sounds are captured. Phone calls, notification sounds and alarms are not. Some apps (such as copy-protected video apps) block audio capture and can't be visualized. In that case **sound is playing but nothing is drawn**, so after a few seconds Home and the running notification tell you that the app does not allow its sound to be shared. The notice clears as soon as sound comes through again. It is not shown when the volume is at zero, during a call, or while headphones or Bluetooth are connected, because then we cannot tell why it is quiet.
...
Check whether the app you're playing blocks audio capture. If it does, Home and the running notification say that the app does not allow its sound to be shared. Also make sure **Opacity** isn't too low in Settings, and that showing the current sound type isn't turned off.

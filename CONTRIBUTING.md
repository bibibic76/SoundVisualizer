...
- 릴리스 빌드는 이름이 바뀌므로(난독화) 크래시 로그를 그대로 읽을 수 없습니다. 되돌릴 때 쓰는 매핑 파일은 `app/build/outputs/mapping/release/mapping.txt` 에 생깁니다. **배포한 APK 의 매핑 파일은 따로 보관하세요.** 다시 빌드하면 덮어써집니다.
- R8 이 지우면 안 되는 것(우리 JNI 진입점, ONNX 런타임이 네이티브에서 이름으로 찾는 클래스)은 `app/proguard-rules.pro` 에 이유와 함께 적혀 있습니다. 그 파일이나 AI 코드, ONNX 런타임 버전을 건드렸으면 **릴리스 APK 를 실제로 설치해 AI 분류가 도는지 확인한 뒤** 머지합니다. (`./gradlew connectedAndroidTest` 의 `...ai` 테스트로도 확인할 수 있습니다.)

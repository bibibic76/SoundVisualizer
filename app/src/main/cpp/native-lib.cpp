...
//
// 순서 보장: producer 는 피크 → pushCount 순으로 쓰고, consumer 는 피크 → pushCount 순으로 읽는다.
...
Java_com_example_soundvisualizer_AudioEngine_readPeaks(JNIEnv *env, jobject thiz,
  ...
  values[2] = static_cast<jfloat>(pushCount.exchange(0, std::memory_order_acq_rel));
...
// 주의: 캡처 스레드가 완전히 종료(join)된 뒤에 호출해야 한다 (AudioCaptureService.onDestroy 가 보장).
extern "C" JNIEXPORT void JNICALL
Java_com_example_soundvisualizer_AudioEngine_destroy(JNIEnv *env,
                                                     jobject thiz) {
  if (audioBuffer != nullptr) {
    delete audioBuffer;
    audioBuffer = nullptr;

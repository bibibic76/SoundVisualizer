...
Java_com_example_soundvisualizer_AudioEngine_pushAudioData(JNIEnv *env,
  ...
  if (c_data != nullptr) {
    ...
    if (frames > 0) {
      ...
      currentLeftRms.store(lRms * 0.2f + currentLeftRms.load() * 0.8f,
                           std::memory_order_relaxed);
      currentRightRms.store(rRms * 0.2f + currentRightRms.load() * 0.8f,
                            std::memory_order_relaxed);
    }
...
Java_com_example_soundvisualizer_AudioEngine_getAudioLevels(JNIEnv *env,
                                                            jobject thiz) {

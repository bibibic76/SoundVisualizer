...
graph TD
    ...
    C -->|"takeHapticPeak"| H["HapticNotifier<br/>진동 알림"]
...
3. **홈 안내**: 알림을 올렸는지와 상관없이 이유를 `SettingsManager.lastUnexpectedStop`에 저장하고, 홈 화면이 상태 아래에 무엇이 꺼졌는지와 **안내 닫기** 버튼을 보여줍니다. 앱 알림을 꺼 두면 알림도 토스트도 뜨지 못해 진동만 남기 때문입니다. 안내가 홈에만 있으므로, 설정·도움말 탭을 보던 중에 꺼졌으면 `MainActivity.onResume`이 홈 탭으로 옮겨 놓습니다. 옮기는 것은 **안내 하나에 한 번뿐**입니다(`routedStopNotice`, 화면 회전·언어 변경으로 다시 만들어져도 유지되게 `onSaveInstanceState`에 저장). 안내를 구분하는 번호도 이유와 함께 저장합니다. 이 값들과 타일 추가 여부는 **그 기기에만 뜻이 있어서**, 저장할 때 기기 표시(`Settings.Secure.ANDROID_ID`)를 함께 남기고 다른 기기에서 복원된 것이면 비웁니다(#181). 처음에는 `Build.FINGERPRINT`를 썼는데 그 값은 **OS 빌드가 바뀌면 달라져서**, 업데이트를 받은 같은 폰을 "다른 기기"로 보고 이 값들을 지웠습니다(#204). `ANDROID_ID`는 OS 업데이트로 바뀌지 않고, 같은 모델·같은 빌드의 두 폰도 구분합니다. 못 읽는 드문 경우에는 예전처럼 `FINGERPRINT`로 물러납니다. 자동 백업은 프리퍼런스 파일을 통째로 옮기고 백업 규칙은 파일 단위라 키 하나만 뺄 수 없습니다. 그대로 두면 새 폰에서 앱을 처음 열었을 때 켠 적도 없는데 꺼짐 안내가 뜹니다. 번호만 프로세스마다 0부터 다시 세면 되살린 "이미 옮긴 번호"와 겹쳐, 설정·도움말 탭에 있던 사용자가 새 안내를 한 번도 못 보고 넘어갑니다(#176). 돌아올 때마다 옮기면 안내를 닫기 전까지는 보던 탭에 남을 수 없고, 설정 탭에서 언어를 바꿀 때마다 홈으로 끌려 나옵니다. 안내가 사라지면 되돌려 다음 안내에 다시 한 번 옮깁니다. 타일 길게 누르기로 설정을 열러 온 경우는 처음부터 옮긴 것으로 쳐서 설정 탭에 그대로 둡니다. 이유는 이름으로 저장하며, 저장·복원 규칙은 `StopNoticeSettingsTest`가 검사합니다. 다시 켜는 버튼은 따로 두지 않습니다(바로 아래 실행 버튼). 다시 켜지면(`onCreate`) 지웁니다.
...
| `currentLevel()` | 가장 최근 버퍼의 좌우 중 큰 값 | 유지 | 개발자 모드 HUD |
| `takePeakSinceLastCheck(out)` | `[피크, 도착한 버퍼 수]` (지난 호출 이후 최대값) | 0으로 초기화 | 보호된 소리 확인 |
| `takeHapticPeak()` | 지난 호출 이후 최대 피크 | 0으로 초기화 | 진동 알림 |

상태는 정적 `std::atomic` 뿐이라 락이 없습니다. **읽으면서 0으로 되돌리는 누적값은 소비자마다 따로 둡니다.** 하나를 나눠 쓰면 먼저 읽는 쪽이 비워, 다른 쪽은 그 구간을 통째로 놓칩니다. 아무 때나 읽어도 되는 `currentLevel`은 가장 최근 버퍼(11.6ms) 하나만 담고 있어서, 0.5초마다 보는 쪽(보호된 소리 확인)이 쓰면 구간의 2% 남짓만 들여다보게 됩니다. **진동도 같은 이유로 누적값을 씁니다**(#174). 0.1초마다 가장 최근 버퍼만 보면 구간의 12% 남짓만 보게 되어, 총소리 한 발처럼 30~60ms 짜리 소리는 확인과 확인 사이에 들어왔다 사라집니다. 화면은 누적값을 읽으니 그려지는데 진동만 빠지는, 가장 나쁜 방향의 실패였습니다. 버퍼당 늘어나는 비용은 원자 연산 세 번입니다.
...
- 파도·외곽선·패드는 **가장자리 띠만** 칠하지만 경로의 범위는 화면 전체라, 그대로 그리면 안티앨리어싱 마스크를 매 프레임 화면 전체 크기로 만듭니다. 그래서 도형이 가장자리에서 가장 깊이 들어온 거리만큼의 **띠 네 개로 잘라** 그립니다(`VisualizerEngine.paintEdgeBand`, #172). 그려지는 화면은 같습니다.
  - 띠 폭은 설정한 최대 깊이가 아니라 실제 파도 점으로 잽니다. 둥근 모서리는 최대 깊이의 약 1.25배까지 들어옵니다. 그릴 때와 같은 곡선을 점 사이까지 짚어 띠 밖으로 나가지 않는지 테스트가 확인합니다.
  - Galaxy S25+ 에서 가짜 소리 크기로 엔진을 계속 그리는 로컬 벤치로 잰 한 코어 기준 값입니다(디버그 빌드, 30초씩).

    | 모드 | 자르기 전 | 자른 뒤 | 프레임 시간 중간값 |
    |---|---:|---:|---|
    | 파도 (기본) | 68% | 45% | 13ms → 6ms |
    | 외곽선 | 89% | 46% | 11ms → 7ms |
    | 패드 | 68% | 26% | 13ms → 5ms |
    | 원형 (해당 없음, 대조군) | 23% | 24% | 5ms |

  - **발광이 켜져 있어도 자릅니다.** 띠 폭에 블러가 번지는 폭(반경의 3배)을 더하고, 발광도 띠 안에서 그립니다. 도형이 띠 안에 있으므로 발광이 번지는 곳까지 띠에 들어오고, 띠 네 개는 겹치지 않게 화면을 나누므로 잘라도 나오는 그림은 같습니다.
    같은 기기에서 발광을 켠 채 12초씩, 두 방식을 한 APK에 넣어 번갈아 재고 순서를 뒤집어 한 번 더 쟀습니다(디버그 빌드). 한 코어 기준입니다.

    | 모드 | 자르기 전 | 자른 뒤 | 늦은 프레임 |
    |---|---:|---:|---|
    | 파도 | 90~91% | 52% | 97% → 33~35% |
    | 외곽선 | 85~90% | 59~62% | 93~97% → 46~55% |
    | 패드 | 87~88% | 31% | 96~97% → 2% |
    | 원형 (띠를 쓰지 않음, 대조군) | 46% / 28% | 45% / 25% | 변화 없음 |

    대조군이 회차 묶음 사이에서 46% → 28% 로 움직인 것은 기기의 다른 부하 때문입니다. 두 방식을 붙여 재고 대조군을 함께 두는 것이 그 흔들림을 가려내는 방법입니다.

    #172 에서는 반대로 보아 발광이면 자르지 않았습니다. **그 측정이 틀렸습니다.** 회차마다 벤치가 앞에서 밀려나 그린 프레임 수가 달라진 것을 걸러 내지 않았습니다. 지금은 회차마다 그린 프레임 수로 회차를 판정하고(화면이 둘인 기기에서는 앞 화면 검사가 맞지 않습니다), 두 방식을 번갈아 재며, 띠를 쓰지 않는 원형 모드를 대조군으로 함께 잽니다. 나오는 그림이 같은지는 같은 정지 화면을 두 방식으로 찍어 픽셀로 비교했습니다(최대 차이 255 중 4, 외곽선만 화면 맨 아랫줄 2픽셀).
...
- **디스크는 쓰는 스레드에서만** 만집니다. 부르는 곳이 오버레이의 HUD 루프(메인 스레드)라서, 거기서 폴더를 확인하거나 파일을 열거나 쓰면 그리는 프레임이 밀립니다. `offer()` 는 큐에 넣고 바로 돌아오고, 큐가 차면 **가장 오래된 줄을 버립니다**(버린 수는 `dropped` 로 남습니다). 기록이 밀리는 것보다 화면이 밀리는 것이 나쁩니다.
  - 파일 경로를 정하는 일까지 쓰는 스레드가 합니다. `getExternalFilesDir()` 은 경로 계산만 하지 않고 대상 폴더를 확인하고(stat) 없으면 만듭니다(#208). 그래서 `currentFile` 은 켠 직후 잠깐 null 입니다.
  - 파일은 **첫 줄이 올 때** 만듭니다. 켜 보고 바로 끈 기기에 빈 파일이 남지 않습니다.
- **켠 회차마다 큐를 따로 둡니다.** `stop()` 은 쓰는 스레드가 끝나기를 기다리지 않습니다 — 메인 스레드를 잡을 수 없기 때문입니다. 그래서 껐다 곧바로 켜면 옛 스레드가 아직 살아 있는데, 큐가 하나면 두 스레드가 그것을 나눠 먹어 줄이 두 파일로 갈렸습니다(#208). 회차마다 큐가 따로면 옛 스레드는 자기 줄만 비우고 나가고(끌 때 남은 줄을 잃지 않습니다), 새 줄은 새 스레드만 봅니다. 상한이나 열기 실패로 스스로 그만둘 때도 자기 회차만 닫습니다.
...
- `SV-Haptic` 스레드(`THREAD_PRIORITY_BACKGROUND`)가 **100ms마다** 최근 분류 라벨과 `AudioEngine.takeHapticPeak()`(지난 틱 이후 구간 전체의 최대 피크)을 읽습니다. AI 결과가 250ms마다 나오므로 놓치지 않습니다.
...
- 사건이 시작되면 울립니다. 단, 같은 종류로 마지막에 울린 지 2초가 안 됐으면 건너뜁니다(연발 총소리의 연타 방지).
- **쿨다운에 막힌 사건은 들고 있다가**, 2초가 지났을 때 그 소리가 아직 나고 있으면 그때 한 번 울립니다(#174). 예전에는 막힌 사건을 잊어서, 총소리 한 발 뒤 1.5초 만에 시작된 경보음이 계속 울리는 동안에도 진동이 없었습니다.
...
| 지난 틱 이후 최대 소리 크기 | 캡처 | `SV-Haptic` (`takeHapticPeak`) | C++ `std::atomic` |
...
- **언어를 바꾸면 실행 중 알림도 다시 만듭니다.** `AppLanguage.changes`가 오르면 서비스가 문구용 컨텍스트를 새로 얻어 알림을 다시 올리고, **알림 채널도 같은 ID로 다시 만듭니다**(채널 이름은 만들 때 정해져, 그러지 않으면 시스템 설정에 옛 언어가 남습니다). 처음에는 12 이하만 신호를 올렸는데(#187), 13 이상은 시스템이 액티비티만 다시 만들고 이미 올라간 알림은 그대로 두기 때문에 옛 언어가 남았습니다(#206). 13 이상에서는 서비스 컨텍스트의 설정이 언제 갱신되는지 보장되지 않아, 고른 언어로 컨텍스트를 직접 만드는 `AppLanguage.localizedContext()`를 씁니다(기본 로캘은 건드리지 않습니다 — 그 버전에서는 시스템이 관리합니다).
